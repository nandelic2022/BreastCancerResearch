...
def generateGPParameters(): 
    ...
    noGen = random.randint(200,300)
    while True: 
        tourSize = random.randint(100,500)
        if tourSize < PopSize:
            break 
        else: 
            pass 
    ...
    while True: 
        ...
        pSubMute = random.uniform(0.95,1)
    ...
    maxSamples = random.uniform(0.99,1)
    constRange = (-random.uniform(0.01,10000), random.uniform(0.01,10000))
    parsimony = random.uniform(.1,1)/10000.0
    parameters = [PopSize, \
                  noGen,\
                  tourSize,\
                  treeDepth,\
...
def GeneticProgramming(genes,X_train,X_test, y_train, y_test):
    ...
    if all(x > 0.99 for x in (MEAN_ACC, MEAN_ROCAUC, MEAN_PRECISION, MEAN_RECALL, MEAN_F1_SCORE)):
        ...
        ACCURACY_FINAL_TRAIN = accuracy_score(y_train, est_gp.predict(X_train))
        ACCURACY_FINAL_TEST = accuracy_score(y_test, est_gp.predict(X_test))
        ROC_AUC_FINAL_TRAIN = roc_auc_score(y_train, est_gp.predict(X_train))
        ROC_AUC_FINAL_TEST = roc_auc_score(y_test, est_gp.predict(X_test))
        PRECISION_FINAL_TRAIN = precision_score(y_train, est_gp.predict(X_train))
        PRECISION_FINAL_TEST = precision_score(y_test, est_gp.predict(X_test))
        RECALL_FINAL_TRAIN = recall_score(y_train, est_gp.predict(X_train))
        RECALL_FINAL_TEST = recall_score(y_test, est_gp.predict(X_test))
        F1_SCORE_FINAL_TRAIN = f1_score(y_train, est_gp.predict(X_train))
        F1_SCORE_FINAL_TEST = f1_score(y_test, est_gp.predict(X_test))
...
###############################################################################
df = pd.read_csv("Simplifed_BC.csv").sample(frac=1) #load the dataset after PCA was performed 
y = df.pop('type_0.0')
df.pop('type_5.0')
df.pop('type_1.0')
df.pop('type_3.0')
...
print(Counter(y_smt))
X_train, X_test, y_train, y_test = train_test_split(X_smt,y_smt, test_size=0.3, random_state = 102)
name="SMOTE_type_0.0"
